...
def setup_worker_environment(n_workers: int, total_cores: int = 48):
...
class BenchmarkRunner:
    def __init__(self, parallel: bool = False, n_workers: int = None, total_cores: int = 48):
        self.parallel = parallel
        self.total_cores = total_cores

        # Auto-determine workers for parallel mode
        if parallel and n_workers is None:
            self.n_workers = min(24, max(12, total_cores // 2))
        else:
